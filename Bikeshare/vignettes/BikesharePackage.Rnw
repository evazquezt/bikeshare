...
distances <- as.data.frame(t(getDistance(stations,from.subset="stationId==31200",
                        mode="bicycling")))
closeStations <- (distances[,1] <= 3) | (rownames(distances)=="31200")
plotTrips(bd, station.subset=closeStations,zoom=14,alpha=.5)
@



plotBubbles(bd, station.subset=closeStations,zoom=14,alpha=.5)

of travel between the Dupont Circle station and the others, we use the \texttt{plotTrips} function.
